...
function:
  var-type ID LP  RP compound-statement{/*ResetSymbolTable();*/
...
init-declarator-list:
  ID {InsertId($1.addr,lineno,type);}
  |ID ASSIGN assignment-expression{InsertId($1.addr,lineno,type);genCode($1.addr,$3.addr);}
  |init-declarator-list COMMA ID{InsertId($3.addr,lineno,type);}
  |init-declarator-list COMMA ID ASSIGN assignment-expression{InsertId($3.addr,lineno,type);}
;
...
switch-statement:
  case-list{$$=$1;}
  |case-list default-statement{$$ = mergeSwitchList($2,$1);}
  |default-statement case-list{$$ = mergeSwitchList($1,$2);}
  |case-list default-statement case-list{$$ = mergeSwitchList(mergeSwitchList($1,$2),$3);}
;
default-statement:
